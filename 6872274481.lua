...
runFunction(function()
    local fartlol = GuiLibrary.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({
        Name = "ScytheTingy",
        Function = function(callback)
            if callback then 
                local Players = game:GetService("Players")
                ...
                -- Use pcall to catch potential errors when fetching objects
                local success, objs = pcall(function()
                    return game:GetObjects("rbxassetid://14347599212")
                end)
                
                if success then
                    -- Check if objs is an array of objects
                    if type(objs) == "table" and #objs > 0 then
                        ...
                        print("No objects found with the given asset ID.")
                    end
                else
                    print("Error fetching objects:", objs)
                end
            end
        end
    })
